...
"""
    construct_brick_log_prior(joint_antarctic_prior::Bool; calibration_data_dir::Union{String, Nothing} = nothing, glacier_model=:gsic)
...
"""
function construct_brick_log_prior(joint_antarctic_prior::Bool; calibration_data_dir::Union{String, Nothing} = nothing, glacier_model=:gsic)
    ...
    function total_log_prior(p::Array{Float64,1})
        ...
        elseif glacier_model==:mengel
            ...
            greenland_α              = p[16]
            greenland_β              = p[17]
            glaciers_a               = p[18] # update
            glaciers_b               = p[19]
            glaciers_T_lia           = p[20]
            glaciers_f               = p[21]
            glaciers_tau_fast        = p[22]
            glaciers_tau_slow        = p[23]
            antarctic_params[:]      = p[24:38]

            log_prior = logpdf(prior_σ_glaciers, σ_glaciers) + logpdf(prior_σ_greenland, σ_greenland) + logpdf(prior_σ_antarctic, σ_antarctic) + logpdf(prior_σ_gmsl, σ_gmsl) +
                        logpdf(prior_ρ_glaciers, ρ_glaciers) + logpdf(prior_ρ_greenland, ρ_greenland) + logpdf(prior_ρ_antarctic, ρ_antarctic) + logpdf(prior_ρ_gmsl, ρ_gmsl) +
                        logpdf(prior_thermal_s₀, thermal_s₀) + logpdf(prior_greenland_v₀, greenland_v₀) + logpdf(prior_glaciers_sl0, glaciers_sl0) + logpdf(prior_antarctic_s₀, antarctic_s₀) +
                        logpdf(prior_thermal_α, thermal_α) +
                        logpdf(prior_greenland_a, greenland_a) + logpdf(prior_greenland_b, greenland_b) + logpdf(prior_greenland_α, greenland_α) + logpdf(prior_greenland_β, greenland_β) +
...
"""
    construct_brick_log_posterior(f_run_model!; model_start_year::Int=1850, calibration_end_year::Int=2017, joint_antarctic_prior::Bool=false, calibration_data_dir::Union{String, Nothing}=nothing, glacier_model=:gsic, gmsl_data::Symbol=:wa, glacier_data::Symbol=:ze)
...
"""
function construct_brick_log_posterior(f_run_model!; model_start_year::Int=1850, calibration_end_year::Int=2017, joint_antarctic_prior::Bool=false, calibration_data_dir::Union{String, Nothing}=nothing, glacier_model=:gsic, gmsl_data::Symbol=:wa, glacier_data::Symbol=:ze)
