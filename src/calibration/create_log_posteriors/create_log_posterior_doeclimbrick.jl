...
"""
    construct_doeclimbrick_log_prior(joint_antarctic_prior::Bool, uniform_ECS::Bool; calibration_data_dir::Union{String, Nothing} = nothing, glacier_model=:gsic)
...
"""
function construct_doeclimbrick_log_prior(joint_antarctic_prior::Bool, uniform_ECS::Bool; calibration_data_dir::Union{String, Nothing} = nothing, glacier_model=:gsic)
    ...
    function total_log_prior(p::Array{Float64,1})
        ...
        elseif glacier_model==:mengel
            ...
            thermal_s₀               = p[15]
            greenland_v₀             = p[16]
            glaciers_sl0             = p[17] # update
            antarctic_s₀             = p[18]
            heat_diffusivity         = p[19]
            rf_scale_aerosol         = p[20]
            ECS                      = p[21]
            thermal_α                = p[22]
            greenland_a              = p[23]
            greenland_b              = p[24]
            greenland_α              = p[25]
            greenland_β              = p[26]
            glaciers_a               = p[27] # update
            glaciers_b               = p[28]
            glaciers_T_lia           = p[29]
            glaciers_f               = p[30]
            glaciers_tau_fast        = p[31]
            glaciers_tau_slow        = p[32]
            antarctic_params[:]      = p[33:47]

            log_prior = logpdf(prior_σ_temperature, σ_temperature) + logpdf(prior_σ_ocean_heat, σ_ocean_heat) + logpdf(prior_σ_glaciers, σ_glaciers) + logpdf(prior_σ_greenland, σ_greenland) + logpdf(prior_σ_antarctic, σ_antarctic) + logpdf(prior_σ_gmsl, σ_gmsl) +
                        logpdf(prior_ρ_temperature, ρ_temperature) + logpdf(prior_ρ_ocean_heat, ρ_ocean_heat) + logpdf(prior_ρ_glaciers, ρ_glaciers) + logpdf(prior_ρ_greenland, ρ_greenland) + logpdf(prior_ρ_antarctic, ρ_antarctic) + logpdf(prior_ρ_gmsl, ρ_gmsl) +
                        logpdf(prior_temperature_0, temperature_0) + logpdf(prior_ocean_heat_0, ocean_heat_0) + logpdf(prior_thermal_s₀, thermal_s₀) + logpdf(prior_greenland_v₀, greenland_v₀) + logpdf(prior_glaciers_sl0, glaciers_sl0) + logpdf(prior_antarctic_s₀, antarctic_s₀) +
                        logpdf(prior_heat_diffusivity, heat_diffusivity) + logpdf(prior_rf_scale_aerosol, rf_scale_aerosol) + logpdf(prior_ECS, ECS) +
                        logpdf(prior_thermal_α, thermal_α) +
                        logpdf(prior_greenland_a, greenland_a) + logpdf(prior_greenland_b, greenland_b) + logpdf(prior_greenland_α, greenland_α) + logpdf(prior_greenland_β, greenland_β) +
...
"""
    construct_doeclimbrick_log_posterior(f_run_model!; model_start_year::Int=1850, calibration_end_year::Int=2017, joint_antarctic_prior::Bool=false, uniform_ECS::Bool=false, calibration_data_dir::Union{String, Nothing}=nothing, glacier_model=:gsic, gmsl_data::Symbol=:wa, glacier_data::Symbol=:ze)
...
"""
function construct_doeclimbrick_log_posterior(f_run_model!; model_start_year::Int=1850, calibration_end_year::Int=2017, joint_antarctic_prior::Bool=false, uniform_ECS::Bool=false, calibration_data_dir::Union{String, Nothing}=nothing, glacier_model=:gsic, gmsl_data::Symbol=:wa, glacier_data::Symbol=:ze)
   ...
   # Create a vector of calibration years and calculate total number of years to run model.
    ...
    calibration_data, obs_antarctic_trends, obs_thermal_trends = MimiBRICK.load_calibration_data(model_start_year, calibration_end_year, last_sea_level_norm_year=1990, calibration_data_dir=calibration_data_dir, gmsl_data=gmsl_data, glacier_data=glacier_data)
