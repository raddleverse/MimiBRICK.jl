...
function run_calibration(;  output_dir::String, 
    ...
    # Check convergence by computing Gelman and Rubin diagnostic for each parameter
    # (potential scale reduction factor). `gelmandiag` is the univariate diagnostic, so it
    # yields one PSRF per parameter just like the previous `potential_scale_reduction` did.
