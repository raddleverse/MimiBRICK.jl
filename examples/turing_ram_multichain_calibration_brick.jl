...
## `src/calibration/turing_ram_calibration_brick.jl`) of the standalone BRICK Turing model in
## parallel worker processes -- the BRICK analogue of `turing_ram_multichain_calibration.jl` (the
## SNEASY-BRICK version). Each chain builds its own Mimi model instance internally (inside
## `run_per_block_ram_calibration_brick`), so there's no shared mutable state between chains --
## separate `Distributed` worker processes make that isolation automatic (and side-step any
## question of whether Mimi/Turing internals are thread-safe, which multiple `Threads.@spawn`'d
## chains sharing one Julia process would raise).
##
## After sampling, this:
...
##   2. Saves the pooled post-burn-in posterior (draw x chain, for each of the 35 BRICK parameters
##      plus the log-joint density) to NetCDF.
...
output_dir = joinpath(@__DIR__, "results", "turing_ram_multichain_brick")
mkpath(output_dir)
...
chain_results = try
    pmap(1:n_chains) do i
        MimiBRICK.run_per_block_ram_calibration_brick(n_samples;
...
## quantity (35 physical/statistical parameters, 16 R_i_j entries, and the log-joint density).
##------------------------------------------------------------------------------
...
ess_path = joinpath(output_dir, "ess_rhat_brick.csv")
...
##------------------------------------------------------------------------------
## 3. Save the pooled post-burn-in posterior to NetCDF: one variable per BRICK parameter (named via
## the package's established external naming convention) plus `log_post`, each with (draw, chain)
## dimensions.
##------------------------------------------------------------------------------
posterior_path = joinpath(output_dir, "posteriors_brick.nc")
isfile(posterior_path) && rm(posterior_path)

internal_names = vcat([string(n) for n in MimiBRICK.BRICK_SCALAR_PARAM_NAMES], ["antarctic_params[$i]" for i in 1:15])
external_names = MimiBRICK.BRICK_EXTERNAL_PARAM_NAMES
...
## `<output_dir>/projections_csv/brick/<scenario>/`, keyed only by `model_config` and
## `ssprcp_scenario` -- not by start/end year -- so the hindcast and projection each need their own
## `output_dir` to avoid one overwriting the other's CSVs. Standalone BRICK has no
## temperature/ocean heat/CO₂ output (those are exogenous forcing, not calibration targets), so its
## field list is shorter than SNEASY-BRICK's.
##------------------------------------------------------------------------------
const PROJECTION_FIELDS = ("gmsl", "landwater_storage_sl", "glaciers", "greenland", "antarctic", "thermal")
...
for (label, run_dir, start_year, end_year, nc_name) in (
    ("hindcast",   joinpath(output_dir, "for_hindcast"),   hindcast_start_year, hindcast_end_year,   "hindcast_brick.nc"),
    ("projection", joinpath(output_dir, "for_projection"), hindcast_start_year, projection_end_year, "projections_brick_$(ssprcp_scenario).nc"),
)
    mkpath(run_dir)
    save(joinpath(run_dir, "parameters_subsample_brick.csv"), parameters_out)
    save(joinpath(run_dir, "log_post_subsample_brick.csv"), DataFrame(log_post = subsample.logjoint))

    println("Running $label ($start_year-$end_year, $ssprcp_scenario)...")
    MimiBRICK.run_projections(output_dir = run_dir, model_config = "brick", ssprcp_scenario = ssprcp_scenario,
        start_year = start_year, end_year = end_year)

    nc_path = consolidate_to_netcdf(run_dir, "brick", ssprcp_scenario, start_year, end_year, joinpath(output_dir, nc_name))
